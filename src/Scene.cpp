...
#define SPECULAR_EXPONENT 1000.0f
#define SPECULAR_EXPONENT_INTEGRAL (SPECULAR_EXPONENT + 1.0f)

gmtl::Rayf* Scene::Bounce(int band, gmtl::Rayf* sound_ray, gmtl::Vec3f*& surface_normal, float& l, Material*& mat, BounceType& bt) {
	gmtl::Point3f* p = 0;
	gmtl::Rayf* old_sound_ray = 0;

	// Only testing intersections with meshes[0] because it contains a combination of all meshes added to the scene
	if ( meshes[0]->RayIntersection(sound_ray,p,surface_normal,mat) ) {
...
gmtl::LineSegf* Scene::Connect(const gmtl::Point3f* p, const gmtl::Point3f& x) {
...
void Scene::Render(int band, int sound, float absorbtion_factor, int num_samples, float dry, const std::vector<Recorder*>& recs, int keyframeID) {
	gmtl::Math::seedRandom((int)time(NULL));

	AbstractSoundFile* currentSound = sources[sound];
	const gmtl::Point3f sfloc = currentSound->getLocation(keyframeID);
	...
	for( int sample_count = 0; sample_count < num_samples; sample_count ++ ) {
		...
		gmtl::Vec3f previous_ray_direction = gmtl::Vec3f();
		...
		for( int num_bounces = 0; num_bounces < 200; num_bounces ++ ) {
			...
			if ( ! sound_ray ) {
				...
				old_sound_ray = Bounce(band,sound_ray,surface_normal,segment_length,mat,bt);
			...
			const float specularity_coefficient = ( (mat > 0) ? mat->specularity_coefficient[band] : 0 );
			...
			// Direct sound is added in a separate step in the end, because in future versions it might
			// be stored separately, for example to be able to reproduce phenomena like Doppler effect.
			// In case the sound source emits from a mesh, the direct sound is sampled regardless.
			if ( num_bounces || currentSound->isMeshSource() ) {

				// For every recorder in the scene..
				for ( std::vector<Recorder*>::const_iterator it = recs.begin(); it != recs.end(); ++ it ) {
					Recorder* rec = *it;

					// See if the intersection point of the ray is 'visible' from the recorder location
					gmtl::LineSegf* ls = Connect(&sound_ray->mOrigin,rec->getLocation(keyframeID));

					if ( ls ) {

						// Because triangles in EAR are two-sided we might need to re-orient the
						// surface normal of the triangle based on its dot product with the line
						// segment direction
						const gmtl::Vec3f lsdir = gmtl::makeNormal(ls->mDir);
						bool valid = true;

						const float dot = num_bounces ? gmtl::dot(lsdir,*surface_normal) : 1.0f;

						if ( dot > 0 ) {
							float this_sample_intensity = sample_intensity_before_bounce;

							// A valid path from the intersection point to the listener location has been
							// found, now we need to determine the intensity of the contribution of the ray.
							if ( num_bounces) {
							if ( bt == REFLECT ) {
								gmtl::Vec3f reflected_vector;
								gmtl::reflect(reflected_vector,previous_ray_direction,*surface_normal);
								const float diffuse_factor = -gmtl::dot(*surface_normal,previous_ray_direction);
								const float specular_factor = (std::max)(0.0f,gmtl::dot(reflected_vector,lsdir));
								const float factor = specularity_coefficient * SPECULAR_EXPONENT_INTEGRAL * pow(specular_factor,SPECULAR_EXPONENT) + (1.0f - specularity_coefficient) * diffuse_factor;
								this_sample_intensity *= factor;
							} else {
								const float diffuse_factor = gmtl::dot(*surface_normal,previous_ray_direction);
								const float specular_factor = (std::max)(0.0f,gmtl::dot(previous_ray_direction,lsdir));
								const float factor = specularity_coefficient * SPECULAR_EXPONENT_INTEGRAL * pow(specular_factor,SPECULAR_EXPONENT) + (1.0f - specularity_coefficient) * diffuse_factor;
								this_sample_intensity *= factor;
							}
							}
							...
							if ( !INVALID_FLOAT(this_sample_intensity) ) {
								rec->Record(lsdir, this_sample_intensity,(total_path_length+l)/343.0f,total_path_length+l, band, keyframeID);
							} 
						}
					}
	...
	for ( std::vector<Recorder*>::const_iterator it = recs.begin(); it != recs.end(); ++ it ) {
		...
		if ( ls ) {
			...
			rec->Record(dir,INV_SPHERE_2(len)*pow(absorbtion_factor,len)*dry,len/343.0f,len, band, keyframeID);
		}
		}
...
Scene::~Scene() {
	for ( std::vector<Recorder*>::const_iterator it = listeners.begin(); it != listeners.end(); ++ it ) {
		delete *it;
	}
